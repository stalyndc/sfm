...
if (!function_exists('sfm_logger_dir')) {
  ...
  function sfm_logger_dir(): string {
    ...
    $publicHtml  = dirname(__DIR__);          // project root
    $accountRoot = dirname($publicHtml);      // hosting account root (if applicable)

    $candidates = [];
    $secureDir = sfm_secure_dir();
    if ($secureDir !== null) {
      $candidates[] = $secureDir . '/logs';   // preferred resolved secure dir
    }
    $candidates[] = $accountRoot . '/secure/logs'; // legacy outside-webroot placement
    $candidates[] = $publicHtml . '/secure/logs';  // local repo fallback
    $candidates[] = $publicHtml . '/logs';         // final in-webroot fallback
    $candidates[] = rtrim(sys_get_temp_dir(), '/\\') . '/sfm-logs'; // last resort
